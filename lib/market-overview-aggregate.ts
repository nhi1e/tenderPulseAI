...
export function aggregateOverviewFacts(
  ...
  facts.forEach((fact) => {
    ...
    const companyName = mappedCompanyName(fact.company, fact.company).trim() || "Chưa xác định";
    ...
    addNamedValue(hospitalEntry.companyTotals, companyName, fact.value, fact.units, true);

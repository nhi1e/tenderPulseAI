...
export function aggregateOverviewFacts(
	...
	facts.forEach((fact) => {
		...
		const competitorHospital = companyDetail.hospitalTotals.get(buyerKey) || {
			name: fact.hospital,
			value: 0,
			units: 0,
		};
	...
	const hospitals = [...hospitalTotals.values()]
		.map(
			({
				productKeys,
				tenderKeys,
				companyTotals: hospitalCompanyTotals,
				...entry
			}) => ({
				...entry,
				...
				competitors: [...hospitalCompanyTotals.values()]
					.filter(
						(company) =>
							...
							company.name !== "Unknown / review needed",
					)
	...
	const companies = [...companyTotals.values()]
		.filter(
			(entry) =>
				entry.name !== "Chưa xác định" &&
				entry.name !== "Unknown / review needed",
		)
		.sort((left, right) => right.value - left.value);
	const competitors = [...companyDetails.values()]
		.filter(
			(entry) =>
				entry.name !== "Medtronic" &&
				entry.name !== "Chưa xác định" &&
				entry.name !== "Unknown / review needed",
		)
		.sort((left, right) => right.value - left.value)
		...
		.map(
			({
				...
				productKeys,
				tenderKeys,
				hospitalTotals: competitorHospitalTotals,
				...entry
			}) => ({
				...entry,
				...
				topHospitals: [...competitorHospitalTotals.values()]
					.sort((left, right) => right.value - left.value)
					.slice(0, 5),
			}),
		);

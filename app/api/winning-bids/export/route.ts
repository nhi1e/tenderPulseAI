...
function filenameDateRange(
	...
	const recordDates = records
		.map((record) => dateKey(record.ngayDangTaiKqlcnt))
		.filter((date): date is string => Boolean(date))
		.sort();
	const dateFrom = filters.dateFrom || recordDates[0];
	const dateTo = filters.dateTo || recordDates.at(-1);

	if (dateFrom && dateTo) {
		return dateFrom === dateTo ? dateFrom : `${dateFrom}-den-${dateTo}`;
	}
	if (dateFrom) return `tu-${dateFrom}`;
	if (dateTo) return `den-${dateTo}`;
	return "tat-ca-thoi-gian";
}
...
function buildWorkbook(
	...
	const rows = records.map((record, index) => {
		...
		return {
			...
			"Hình thức LCNT": safeText(
				record.bidForm ? bidFormNames[record.bidForm] || record.bidForm : "",
			),
		...
		[
			...
			safeText(
				filters.company && filters.company !== "all"
					? companyDisplayNames[filters.company] || filters.company
					: "Tất cả",
			),
		],
...
export async function POST(request: Request) {
	try {
		...
		if (!result.records.length) {
			return Response.json(
				{ error: "Không có kết quả để xuất Excel." },
				{ status: 404 },
			);
		}
		...
		const dateRange = filenameDateRange(filters, result.records);
		const filename = `tenderpulse-${filenamePart(keyword)}-${dateRange}.xlsx`;
		...
		return Response.json(
			{
				error:
					"Không thể tạo file Excel từ dữ liệu trực tiếp. Vui lòng thử lại.",
			},
			{ status: 502 },
		);
	}
